...
namespace UnityEditor.Scripting.Python
{
    ...
    public static class PythonRunner
    {
        ...
        /// <param name="scopeName">Value to write to Python special variable `__name__`</param>
        public static void RunFile(string pythonFileToExecute, string scopeName = null, Dictionary<string, object> scopeOriginal = null)
        {
            EnsureInitialized();
            ...
            using (Py.GIL ())
            {
                ...
                else
                {
                    using (PyModule scope = Py.CreateScope())
                    {
                        ...
                        foreach(KeyValuePair<string, object> ele in scopeOriginal)
                        {
                            scope.Set(ele.Key, ele.Value);
                        }
